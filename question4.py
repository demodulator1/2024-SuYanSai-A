...
def objective(x):
    ...
    # 根据权重调整
    weighted_score = (fracture_normalized + elongation_normalized + tear_normalized +
                      2 * air_perm_normalized + 2 * moisture_perm_normalized +
                      soft_normalized + wrinkle_normalized) / 8
...
print(f"断裂伸长量: {elongation(*best_params)*100}")
